...
namespace Aplicacao.Domain.Services
{
    public class OrderService : BaseService<Order, int>, IOrderService
    {
        ...
        public override async Task<Order> Add(Order entity)
        {
            ...
            var  Customer = await _customerSQLServerRepository.ReadById(idCustomer);
            ...
            if (entity.OrderItems is null)
            {
                entity.ValidationResult.Errors.Add(new ValidationFailure("O cliente não posui itens de venda", $"Cliente id {idCustomer}"));
            }
